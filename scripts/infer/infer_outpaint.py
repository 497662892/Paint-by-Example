...
from ldm.data.infer.outpaint_infer import OpenImageDataset
import clip
...
def main():
    ...
    parser.add_argument(
        "--random_shift",
        type=bool,
        help="whether to use random shift",
        default=False
    )
    ...
    test_dataset=OpenImageDataset(state="train_10", random= opt.random_shift,  dataset_dir = dataset_path, image_size = opt.H)
                    ...
                    for test_model_kwargs in test_dataloader:
                        ...
                        if not opt.skip_save:
                            for i,x_sample in enumerate(x_checked_image_torch):
                                ...
                                img.save(os.path.join(grid_path, 'grid-'+ str(i) + "-" + str(batch) + "-" + ref_name + '.png'))
